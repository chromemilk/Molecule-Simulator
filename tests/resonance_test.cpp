...
int main() {
...

    {
        Generator gen({"O","O","O"});
        auto bonds = gen.bestStructure();
        bool dbl01=false,sng12=false;
        for (auto [i,j,o]:bonds)
        {
            if ((i==0 && j==1) || (i==1 && j==0)) { assert(o==2); dbl01=true; }
            if ((i==1 && j==2) || (i==2 && j==1)) { assert(o==1); sng12=true; }
        }
        assert(dbl01 && sng12 && bonds.size()==2); // ozone orientation
    }
